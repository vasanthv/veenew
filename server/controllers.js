...
const stripeWebhook = async (req, res, next) => {
	try {
		if (!stripe || !config.STRIPE_WEBHOOK_SECRET) return utils.httpError(500, "Stripe webhook is not configured");

		const signature = req.headers["stripe-signature"];
		if (!signature) return utils.httpError(400, "Missing Stripe signature");
		...
		if (event.type === "checkout.session.completed") {
			...
			if (session.mode === "payment" && session.payment_status === "paid" && sessionUserId) {
				await Users.updateOne({ _id: sessionUserId }, { usertype: "paid", updatedOn: new Date() });
			}
		}
